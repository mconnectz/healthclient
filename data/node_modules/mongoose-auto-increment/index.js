...
exports.initialize = function (connection) {
  try {
    Counters = connection.model('Counters');
  } catch (ex) {
...
exports.plugin = function (schema, options) {
  ...
  // If we don't have reference to the counterSchema or the Counters model then the plugin was most likely not
  // initialized properly so throw an error.
  if (!counterSchema || !Counters) throw new Error("mongoose-auto-increment has not been initialized");
  ...
  Counters.findOne(
    ...
    function (err, counter) {
      if (!counter) {
        ...
        counter = new Counters({ model: settings.model, field: settings.field, count: settings.startAt - settings.incrementBy });
      ...
      (function save() {
        ...
        if (ready) {
          ...
          if (typeof doc[settings.field] === 'number') {
            Counters.findOneAndUpdate(
              // Counters documents are identified by the model and field that the plugin was invoked for.
            ...
            Counters.findOneAndUpdate(
              ...
              function (err, updatedCounters) {
                if (err) return next(err);
                // If there are no errors then go ahead and set the document's field to the current count.
                doc[settings.field] = updatedCounters.count;
                // Continue with default document save functionality.
                next();
              }
            );
